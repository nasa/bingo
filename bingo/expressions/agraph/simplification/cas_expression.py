...
class CASExpression:
    ...
    def exponent(self):
        ...
        if self._operator == INTEGER:
            return None
        return CASExpression(INTEGER, [1])
    ...
    def term(self):
        ...
        if self._operator == MULTIPLICATION:
            if self._operands[0].operator in [INTEGER, CONSTANT]:
                return CASExpression(MULTIPLICATION, self._operands[1:])
            return self
        if self._operator == INTEGER:
            return None
        return CASExpression(MULTIPLICATION, [self])

    @property
    def coefficient(self):
        """The coefficient *A* in *A·x*."""
        if (
            self._operator == MULTIPLICATION
            and self._operands[0].operator in [INTEGER, CONSTANT]
        ):
            return self._operands[0]
        if self._operator == INTEGER:
            return None
    ...
    def _operands_lt(s_operands, o_operands):
        for s_operand, o_operand in zip(
            reversed(s_operands), reversed(o_operands)
        ):
    ...
    def _power_lt(self, other):
        if self._operator == POWER:
            ...
            s_base = self
            s_exponent = CASExpression(INTEGER, [1])
        ...
        if other.operator == POWER:
            ...
            o_base = other
            o_exponent = CASExpression(INTEGER, [1])
