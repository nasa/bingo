...
def mutation_onemax_specific():
    ...
    This is the function that will be passed into the MultipleValueMutation function. 
    Any function passed to MultipleValueMutation must return a single random value,
    which will then be used to replace a single value in a chromosome undergoing mutation.
    Returns
    -------
    bool : boolean 
...
def get_random_list_for_chromosome(number_of_values):
    ...
    a list of booleans, although integer and floating point values would also suffice.
