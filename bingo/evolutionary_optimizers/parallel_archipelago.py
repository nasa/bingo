...
class ParallelArchipelago(Archipelago):
    ...
    """
    def __init__(self, template_island, hall_of_fame=None, non_blocking=True,
                 sync_frequency=10):
        self.comm = MPI.COMM_WORLD
        self.comm_rank = self.comm.Get_rank()
        self.comm_size = self.comm.Get_size()
        super().__init__(template_island, self.comm_size, hall_of_fame)
        self._non_blocking = non_blocking
        self._sync_frequency = sync_frequency
        if self._template_island.hall_of_fame is None:
            self._template_island.hall_of_fame = deepcopy(self.hall_of_fame)
    ...
    def get_best_fitness(self):
        ...
        """
        best_on_proc = self._template_island.get_best_fitness()
    ...
    def get_best_individual(self):
        ...
        """
        best_on_proc = self._template_island.get_best_individual()
    ...
    def _step_through_generations(self, num_steps):
        if self._non_blocking:
            ...
            self._template_island.evolve(num_steps,
    ...
    def _non_blocking_execution_master(self, num_steps):
        ...
        while average_age < target_age:
            self._template_island.evolve(self._sync_frequency,
                                         hall_of_fame_update=False,
                                         suppress_logging=True)
    ...
    def _gather_updated_ages(self, total_age):
        total_age.update({0: self._template_island.generational_age})
        status = MPI.Status()
    ...
    def _send_updated_age(self):
        data = {self.comm_rank: self._template_island.generational_age}
    ...
    def _population_exchange_program(self, partner):
        population_to_send = self._template_island.dump_fraction_of_population(0.5)
        received_population = self.comm.sendrecv(population_to_send,
                                                 dest=partner,
                                                 sendtag=MIGRATION,
                                                 source=partner,
                                                 recvtag=MIGRATION)
        self._template_island.population += received_population

    def _log_evolution(self, start_time):
        elapsed_time = datetime.now() - start_time
        LOGGER.log(DETAILED_INFO, "Evolution time %s\t age %d\t fitness %.3le",
                   elapsed_time, self._template_island.generational_age,
                   self.get_best_fitness())

    def _get_potential_hof_members(self):
        self._template_island.update_hall_of_fame()
        potential_members = [i for i in self._template_island.hall_of_fame]
    ...
    def get_fitness_evaluation_count(self):
        ...
        """
        my_eval_count = self._template_island.get_fitness_evaluation_count()
    ...
    def get_ea_diagnostic_info(self):
        ...
        """
        my_diagnostics = self._template_island.get_ea_diagnostic_info()
