...
class EaDiagnostics:
    ...
    def __init__(self, crossover_types=None, mutation_types=None):
        ...
        self._crossover_mutation_type_stats = {
            type_pair: np.zeros(3)
            for type_pair in product(crossover_types, mutation_types)
        }
    ...
    def crossover_mutation_type_summary(self):
        ...
        mutation happened"""
        summary = {}
        for type_pairing in product(
            self._crossover_types, self._mutation_types
        ):
    ...
    def _get_stats(self, idx, beneficial_var, detrimental_var):
        return np.sum(
            [idx, beneficial_var * idx, detrimental_var * idx], axis=1
        )
    ...
    def get_log_header(self):
        ...
        for c in self._crossover_type_stats:
            header += f", {c}_number, {c}_beneficial, {c}_detrimental"
        for m in self._mutation_type_stats:
            header += f", {m}_number, {m}_beneficial, {m}_detrimental"
        for c, m in self._crossover_mutation_type_stats:
            header += (
    ...
    def get_log_stats(self):
        ...
        stats = np.array(
            [
                self._crossover_stats,
                self._mutation_stats,
                self._cross_mut_stats,
            ]
            + list(self._crossover_type_stats.values())
            + list(self._mutation_type_stats.values())
            + list(self._crossover_mutation_type_stats.values()),
            dtype=int,
        ).flatten()
        return stats
        ...
        for crossover_type in self._crossover_types:
            ...
            for mutation_type in self._mutation_types:
                ...
                self._crossover_mutation_type_stats[
                    ...
                    cross_mut[cross_mut_idx],
                    beneficial_var[cross_mut_idx],
