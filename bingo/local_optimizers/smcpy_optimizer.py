...
class SmcpyOptimizer(LocalOptimizer):
    ...
    """
    def __init__(self, objective_fn, deterministic_optimizer,
    ...
    def __call__(self, individual):
        ...
        vector_mcmc = VectorMCMC(lambda x: self.evaluate_model(x, individual),
                                 np.zeros(len(self.training_data)), priors,
    ...
    def _generate_proposal_samples(self, individual, num_samples):
        ...
        if self._std is None:
            ...
            scale_data = np.sqrt(np.mean(np.square(self.training_data.y)))  #TODO can we do this differently without knowing what the training data is?
            noise_dists = []
    ...
    def evaluate_model(self, params, individual):
        ...
        return self._objective_fn.evaluate_fitness_vector(individual).T