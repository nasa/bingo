...
def mut_type_complex():
    return np.array(["m_n"] * 2 + ["m_s"] * 2 + ["m_n"] * 2 + [None] * 2 +
                    ["m_n"] * 2 + [None] * 6, dtype=object)


def test_correctly_updated_overall_summary(population_12,
                                           population_0123_times_4,
                                           cross_type_simple, mut_type_simple):
...
def test_correctly_updated_type_summaries(population_12,
    ...
    for cross_type in expected_cross_summary.keys():
        np.testing.assert_array_equal(ead.crossover_type_summary[cross_type],
    ...
    for mut_type in expected_mut_summary.keys():
        np.testing.assert_array_equal(ead.mutation_type_summary[mut_type],
    ...
    assert ead.crossover_mutation_type_summary.keys() == \
           expected_cross_mut_summary.keys()
    for pair in expected_cross_mut_summary.keys():
        np.testing.assert_array_equal(ead.crossover_mutation_type_summary[pair],
                                      expected_cross_mut_summary[pair])


def test_correctly_updated_existing_type_summaries(population_12,
                                                   population_0123_times_4,
                                                   cross_type_complex,
                                                   mut_type_complex):
    ...
    expected_cross_summary = {
        "c_n": GeneticOperatorSummary(beneficial_rate=np.nan,
                                      detrimental_rate=np.nan),
        "c_s": GeneticOperatorSummary(beneficial_rate=0.25,
                                      detrimental_rate=0.5)}
